/** 
@summary: Custom functions to convert the accounts module record to deals module getRecordById 
@author: Cube Yogi
@created: 23 Dec 2017
**/
...
/** Important Notes : In Zoho CRM Custom Function, 'input' which indicates the input parameters variable, which is configured in custom function by choose the 'Edit Arguements' button
	By Mapping the 'Account Id' value in parameters in custom function
	1. 'Account Id' which is unique id of the accounts record, it is generated by zoho crm system when the accounts record created
	    Using the unique id, get the Accounts record and fetch the contact for the related accounts all field values and process it

**/

/**
	------------------------------------------------------------------------------------------
			CREATE OR INSERT RECORD TO POTENTIAL/DEAL MODULE FROM ACCOUNT MODULE
	------------------------------------------------------------------------------------------
**/

/**
	~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	  Step 1 : Get account name by using the account unique id value, 
...
/** Validate the input account value, which is not null and valid integer value **/
if(input.account_id != null && input.account_id > 0)
{
	/** Get account name from the account id **/
	account_details_response = zoho.crm.getRecordById("Accounts", input.account_id.toLong());
	/** Get the Account name]**/
	account_name = ifnull(account_details_response.get("Account Name"),"");
	/** Get Contact records fron the account name **/
	contact_details_response_list = zoho.crm.searchRecords("Contacts","(Account Name|=|" + account_name + ")");
	if(contact_details_response_list.size() > 0)
	{
		contact_details_response = contact_details_response_list.get(0);
		contact_id = contact_details_response.get("CONTACTID");
		contact_id_str = contact_id.toString();
		...
		**/
		...
		deal_record_fields_map.put("CONTACTID",contact_id);
		deal_record_fields_map.put("Account Name",ifnull(account_name,""));
