/* 
* Zoho CRM Custom Function For Contact Phone Num Country Code Updation
* This Custom Function is for the Contact module in Zoho CRM.
* This code is in Deluge scripting language.
* This is used to check an incoming Contact to see if their phone num has the country dialing code.
* If not, then this code would fetch the appropriate code based on the 'Country' field value from the Contact.
* Then this would update the 'Phone' field in the CRM Contact to reflect the new number which has the country code prefixed.
*
...
if((input.phone  !=  null)  &&  (input.country  !=  null))
{
	...
	if(extension_code_to_add  !=  null)
	{
		...
		// logic to find if there is any leading zeros
		if(((phone.toString()).indexOf("0"))==0)
		{
			input_phone_number=((phone.toString()).removeFirstOccurence("0"));
		}
		...
		if(!startsWith(input.phone,phone_no_to_update)  &&  !startsWith(input.phone,phone_number_sign))
		{
			...
			update_phone_no_in_crm = zoho.crm.updateRecord("Contacts","" + input.contact_id,update_phone_no_map);
		}
	}
}
